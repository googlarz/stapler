...
- `pnpm stapler plugin list`
- `pnpm stapler plugin install <package[@version]>`
- `pnpm stapler plugin uninstall <plugin-id>`
- `pnpm stapler plugin upgrade <plugin-id> [version]`
- `pnpm stapler plugin doctor <plugin-id>`
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@stapler/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@stapler/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@stapler/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@stapler/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@stapler/plugin-sdk/ui`
...
The `@stapler/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
6. The operator may force-purge immediately via CLI: `pnpm stapler plugin purge <plugin-id>`.
...
### 27.1 `@stapler/plugin-test-harness`
...
```ts
import { createTestHarness } from "@stapler/plugin-test-harness";
...
- The operator installs the plugin from a local path: `pnpm stapler plugin install ./path/to/plugin`
...
- `@stapler/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@stapler/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@stapler/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@stapler/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@stapler/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@stapler/plugin-sdk/ui`
