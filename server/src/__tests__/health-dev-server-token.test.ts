...
describe("GET /health dev-server supervisor access", () => {
  it("exposes dev-server metadata to the supervising dev runner in authenticated mode", async () => {
    const previousFile = process.env.STAPLER_DEV_SERVER_STATUS_FILE;
    const previousToken = process.env.STAPLER_DEV_SERVER_STATUS_TOKEN;
    process.env.STAPLER_DEV_SERVER_STATUS_FILE = createDevServerStatusFile({
      dirty: true,
    ...
    process.env.STAPLER_DEV_SERVER_STATUS_TOKEN = "dev-runner-token";
    ...
    try {
      ...
      if (previousFile === undefined) {
        delete process.env.STAPLER_DEV_SERVER_STATUS_FILE;
      } else {
        process.env.STAPLER_DEV_SERVER_STATUS_FILE = previousFile;
      }
      if (previousToken === undefined) {
        delete process.env.STAPLER_DEV_SERVER_STATUS_TOKEN;
      } else {
        process.env.STAPLER_DEV_SERVER_STATUS_TOKEN = previousToken;
      }
    }
  });
});
