...
function resolveEnvLabStatePath(): string {
  const instanceRoot =
    process.env.STAPLER_INSTANCE_ROOT?.trim() ||
...
/** Attempt to build config from explicit STAPLER_ENV_LIVE_SSH_* env vars. */
function tryExplicitConfig(): {
  ...
  const host = process.env.STAPLER_ENV_LIVE_SSH_HOST?.trim() ?? "";
  const username = process.env.STAPLER_ENV_LIVE_SSH_USERNAME?.trim() ?? "";
  const remoteWorkspacePath =
    process.env.STAPLER_ENV_LIVE_SSH_REMOTE_WORKSPACE_PATH?.trim() ?? "";
  const port = Number.parseInt(process.env.STAPLER_ENV_LIVE_SSH_PORT ?? "22", 10);
 ...
 * 1. Explicit STAPLER_ENV_LIVE_SSH_* env vars
...
async function resolveSshConfig(): Promise<SshConnectionConfig | null> {
  ...
  if (explicit) {
    return {
      ...
      privateKey: await readOptionalSecret(
        process.env.STAPLER_ENV_LIVE_SSH_PRIVATE_KEY,
        process.env.STAPLER_ENV_LIVE_SSH_PRIVATE_KEY_PATH,
      ),
      knownHosts: await readOptionalSecret(
        process.env.STAPLER_ENV_LIVE_SSH_KNOWN_HOSTS,
        process.env.STAPLER_ENV_LIVE_SSH_KNOWN_HOSTS_PATH,
      ),
      strictHostKeyChecking:
        (process.env.STAPLER_ENV_LIVE_SSH_STRICT_HOST_KEY_CHECKING ?? "true").toLowerCase() !== "false",
    };
  }
  ...
  if (process.env.STAPLER_ENV_LIVE_SSH_NO_AUTO_FIXTURE !== "true") {
...
const describeLiveSsh = (() => {
  ...
  if (process.env.STAPLER_ENV_LIVE_SSH_NO_AUTO_FIXTURE === "true") {
    console.warn(
      "Skipping live SSH smoke test. Set STAPLER_ENV_LIVE_SSH_HOST, STAPLER_ENV_LIVE_SSH_USERNAME, and STAPLER_ENV_LIVE_SSH_REMOTE_WORKSPACE_PATH to enable it, or remove STAPLER_ENV_LIVE_SSH_NO_AUTO_FIXTURE to auto-start env-lab.",
    );
...
describeLiveSsh("live SSH environment smoke", () => {
  ...
  it("connects to the configured SSH environment and verifies basic runtime tools", async () => {
    ...
    if (!resolvedConfig) {
      throw new Error(
        "Live SSH smoke test could not resolve SSH config from env vars or env-lab fixture. Set STAPLER_ENV_LIVE_SSH_NO_AUTO_FIXTURE=true to mark this suite skipped intentionally.",
      );
    }
