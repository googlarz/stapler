...
const ORIGINAL_STAPLER_RUNTIME_API_CANDIDATES_JSON = process.env.STAPLER_RUNTIME_API_CANDIDATES_JSON;
...
describe("startServer STAPLER_API_URL handling", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    loadConfigMock.mockReturnValue(buildTestConfig());
    process.env.BETTER_AUTH_SECRET = "test-secret";
  ...
  afterEach(() => {
    ...
    if (ORIGINAL_STAPLER_RUNTIME_API_CANDIDATES_JSON === undefined) {
      delete process.env.STAPLER_RUNTIME_API_CANDIDATES_JSON;
    } else {
      process.env.STAPLER_RUNTIME_API_CANDIDATES_JSON = ORIGINAL_STAPLER_RUNTIME_API_CANDIDATES_JSON;
    }
  ...
  it("uses the externally set STAPLER_API_URL when provided", async () => {
    process.env.STAPLER_API_URL = "http://custom-api:3100";

    const started = await startServer();

    expect(started.apiUrl).toBe("http://custom-api:3100");
    expect(process.env.STAPLER_API_URL).toBe("http://custom-api:3100");
    expect(JSON.parse(process.env.STAPLER_RUNTIME_API_CANDIDATES_JSON ?? "[]")).toEqual(
      expect.arrayContaining(["http://custom-api:3100"]),
    );
    expect(JSON.parse(process.env.STAPLER_RUNTIME_API_CANDIDATES_JSON ?? "[]")[0]).toBe("http://custom-api:3100");
  });

  it("falls back to host-based URL when STAPLER_API_URL is not set", async () => {
    const started = await startServer();

    expect(started.apiUrl).toBe("http://127.0.0.1:3210");
    expect(process.env.STAPLER_API_URL).toBe("http://127.0.0.1:3210");
  });
  ...
  it("rewrites explicit-port auth public URLs when detect-port selects a new port", async () => {
    ...
    expect(process.env.STAPLER_RUNTIME_API_URL).toBe("http://my-host.ts.net:3110");
  });
  ...
  it("keeps no-port auth public URLs stable when detect-port selects a new port", async () => {
    ...
    expect(process.env.STAPLER_RUNTIME_API_URL).toBe("https://paperclip.example");
  });
});
