...
const ORIGINAL_INSTANCE_ID = process.env.STAPLER_INSTANCE_ID;

afterEach(() => {
  if (ORIGINAL_INSTANCE_ID === undefined) delete process.env.STAPLER_INSTANCE_ID;
  else process.env.STAPLER_INSTANCE_ID = ORIGINAL_INSTANCE_ID;
});
...
describe("Better Auth cookie scoping", () => {
  ...
  it("uses STAPLER_INSTANCE_ID for the Better Auth cookie prefix", () => {
    process.env.STAPLER_INSTANCE_ID = "sat-worktree";
  ...
  it("keeps local http auth cookies non-secure while preserving the scoped prefix", () => {
    process.env.STAPLER_INSTANCE_ID = "pap-worktree";
