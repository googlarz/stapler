...
## 2. Shared Types (`@stapler/adapter-utils`)

All adapter interfaces live in `packages/adapter-utils/src/types.ts`. Import from `@stapler/adapter-utils` (types) or `@stapler/adapter-utils/server-utils` (runtime helpers).
...
```json
{
  ...
  "dependencies": {
    "@stapler/adapter-utils": "workspace:*",
    "picocolors": "^1.1.1"
  },
  "devDependencies": {
    "typescript": "^5.7.3"
...
1. **Read config** — extract typed values from `ctx.config` using helpers (`asString`, `asNumber`, `asBoolean`, `asStringArray`, `parseObject` from `@stapler/adapter-utils/server-utils`)
2. **Build environment** — call `buildPaperclipEnv(agent)` then layer in `STAPLER_RUN_ID`, context vars (`STAPLER_TASK_ID`, `STAPLER_WAKE_REASON`, `STAPLER_WAKE_COMMENT_ID`, `STAPLER_APPROVAL_ID`, `STAPLER_APPROVAL_STATUS`, `STAPLER_LINKED_ISSUE_IDS`), user env overrides, and auth token
...
| `STAPLER_API_URL` | Server's own URL |
| `STAPLER_RUN_ID` | Current run id |
| `STAPLER_TASK_ID` | `context.taskId` or `context.issueId` |
| `STAPLER_WAKE_REASON` | `context.wakeReason` |
| `STAPLER_WAKE_COMMENT_ID` | `context.wakeCommentId` or `context.commentId` |
| `STAPLER_APPROVAL_ID` | `context.approvalId` |
| `STAPLER_APPROVAL_STATUS` | `context.approvalStatus` |
| `STAPLER_LINKED_ISSUE_IDS` | `context.issueIds` (comma-separated) |
| `STAPLER_API_KEY` | `authToken` (if no explicit key in config) |
...
```ts
import { execute as myExecute, sessionCodec as mySessionCodec } from "@stapler/adapter-my-agent/server";
import { agentConfigurationDoc as myDoc, models as myModels } from "@stapler/adapter-my-agent";
...
import { parseMyAgentStdoutLine } from "@stapler/adapter-my-agent/ui";
import { MyAgentConfigFields } from "./config-fields";
import { buildMyAgentConfig } from "@stapler/adapter-my-agent/ui";
...
```ts
import { printMyAgentStreamEvent } from "@stapler/adapter-my-agent/cli";
...
Import from `@stapler/adapter-utils/server-utils`:
...
| `buildPaperclipEnv(agent)` | Standard `STAPLER_*` env vars |
...
- Package name: `@stapler/adapter-<kebab-name>`
...
async function buildSkillsDir(): Promise<string> {
  ...
  const entries = await fs.readdir(STAPLER_SKILLS_DIR, { withFileTypes: true });
  for (const entry of entries) {
    if (entry.isDirectory()) {
      await fs.symlink(
        path.join(STAPLER_SKILLS_DIR, entry.name),
...
- `STAPLER_API_KEY` is injected by the server into the process environment, not the prompt
