...
function renderPaperclipEnvNote(env: Record<string, string>): string {
  ...
  return [
    ...
    `The following STAPLER_* environment variables are available in this run: ${paperclipKeys.join(", ")}`,
...
function renderApiAccessNote(env: Record<string, string>): string {
  if (!hasNonEmptyEnvValue(env, "STAPLER_API_URL") || !hasNonEmptyEnvValue(env, "STAPLER_API_KEY")) return "";
  return [
    "Paperclip API access note:",
    "Use run_shell_command with curl to make Paperclip API requests.",
    "GET example:",
    `  run_shell_command({ command: "curl -s -H \\"Authorization: Bearer $STAPLER_API_KEY\\" \\"$STAPLER_API_URL/api/agents/me\\"" })`,
    "POST/PATCH example:",
    `  run_shell_command({ command: "curl -s -X POST -H \\"Authorization: Bearer $STAPLER_API_KEY\\" -H 'Content-Type: application/json' -H \\"X-Paperclip-Run-Id: $STAPLER_RUN_ID\\" -d '{...}' \\"$STAPLER_API_URL/api/issues/{id}/checkout\\"" })`,
    "",
    "",
  ].join("\n");
}
