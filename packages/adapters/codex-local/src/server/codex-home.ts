...
function isWorktreeMode(env: NodeJS.ProcessEnv): boolean {
  return TRUTHY_ENV_RE.test(env.STAPLER_IN_WORKTREE ?? "");
}
...
export async function prepareManagedCodexHome(
  ...
  await onLog(
    "stdout",
    `[stapler] Using ${isWorktreeMode(env) ? "worktree-isolated" : "Paperclip-managed"} Codex home "${targetHome}" (seeded from "${sourceHome}").\n`,
  );
  return targetHome;
}
