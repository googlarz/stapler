...
async function pruneBrokenUnavailablePaperclipSkillSymlinks(
  ...
  for (const entry of entries) {
    ...
    await onLog(
      "stdout",
      `[stapler] Removed stale Codex skill "${entry.name}" from ${skillsHome}\n`,
    );
  }
}
    ...
    try {
      ...
      if (existing?.isSymbolicLink()) {
        ...
        if (
          ...
          await onLog(
            "stdout",
            `[stapler] Repaired Codex skill "${entry.runtimeName}" into ${skillsHome}\n`,
          );
          continue;
        }
      }
      ...
      await onLog(
        "stdout",
        `[stapler] ${result === "repaired" ? "Repaired" : "Injected"} Codex skill "${entry.runtimeName}" into ${skillsHome}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[stapler] Failed to inject Codex skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  if (workspaceStrategy) {
    env.STAPLER_WORKSPACE_STRATEGY = workspaceStrategy;
  }
  if (workspaceId) {
  ...
  if (workspaceBranch) {
    env.STAPLER_WORKSPACE_BRANCH = workspaceBranch;
  }
  if (workspaceWorktreePath) {
    env.STAPLER_WORKSPACE_WORKTREE_PATH = workspaceWorktreePath;
  }
  if (agentHome) {
  ...
  // timeoutSec: undefined/missing → default (prevents hangs on new agents, #3173).
  //             explicit 0        → unlimited (preserves existing semantics).
  //             explicit positive → use as-is.
  const timeoutSec =
  ...
  if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stdout",
      `[stapler] Codex session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${cwd}".\n`,
    );
  }
  ...
  if (
    ...
    await onLog(
      "stdout",
      `[stapler] Codex resume session "${sessionId}" is unavailable; retrying with a fresh session.\n`,
    );
