...
**Step 4 — Pick work.** Priority: `in_progress` → `in_review` (if woken by a comment on it — check `STAPLER_WAKE_COMMENT_ID`) → `todo`. Skip `blocked` unless you can unblock.

Overrides and special cases:

- `STAPLER_TASK_ID` set and assigned to you → prioritize that task first.
- `STAPLER_WAKE_REASON=issue_commented` with `STAPLER_WAKE_COMMENT_ID` → read the comment, then checkout and address the feedback (applies to `in_review` too).
- `STAPLER_WAKE_REASON=issue_comment_mentioned` → read the comment thread first even if you're not the assignee. Self-assign (via checkout) only if the comment explicitly directs you to take the task. Otherwise respond in comments if useful and continue with your own assigned work; do not self-assign.
...
If `STAPLER_WAKE_PAYLOAD_JSON` is present, inspect that payload before calling the API. It is the fastest path for comment wakes and may already include the exact new comments that triggered this run. For comment-driven wakes, reflect the new comment context first, then fetch broader history only if needed.
...
- `STAPLER_WAKE_REASON=issue_blockers_resolved` — all `blockedBy` issues reached `done`; dependent's assignee is woken.
- `STAPLER_WAKE_REASON=issue_children_completed` — all direct children reached a terminal state (`done`/`cancelled`); parent's assignee is woken.
...
`issueIds` links the approval into the issue thread. When approved, Paperclip wakes the requester with `STAPLER_APPROVAL_ID`/`STAPLER_APPROVAL_STATUS`. Keep the payload concise and decision-ready.
...
- **Self-assign only for explicit @-mention handoff.** Requires a mention-triggered wake with `STAPLER_WAKE_COMMENT_ID` and a comment that clearly directs you to do the task. Use checkout (never direct assignee patch).
