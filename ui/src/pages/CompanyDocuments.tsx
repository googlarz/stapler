...
export function CompanyDocuments() {
  ...
  const [deleteArmedDocumentId, setDeleteArmedDocumentId] = useState<string | null>(null);
  ...
  const documentNotFound = Boolean(documentId && !creatingNew && documents.length > 0 && !selectedDocument);

  useEffect(() => {
    setDeleteArmedDocumentId(null);
  }, [selectedDocument?.id, creatingNew]);
  ...
  function requestDelete() {
    ...
    if (deleteArmedDocumentId !== selectedDocument.id) {
      setDeleteArmedDocumentId(selectedDocument.id);
      return;
    }
    ...
    <div className="grid gap-6 lg:grid-cols-[280px_minmax(0,1fr)]">
      <section className="space-y-3">
        ...
        <p className="text-sm text-muted-foreground">
          ...
          QA plans, and staff-managed guidance.
        </p>
        ...
        {documents.length === 0 && !creatingNew ? (
          ...
          <div className="rounded-lg border border-border bg-card p-6">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              This document may have been deleted or moved. Select another document from the list.
            </p>
          </div>
        ) : (
          <>
                ...
                {!creatingNew && selectedDocument ? (
                  ...
                  >
                    ...
                    {deleteDocument.isPending
                      ? "Deleting..."
                      : deleteArmedDocumentId === selectedDocument.id
                        ? "Confirm delete"
                        : "Delete"}
                  </Button>
                ) : null}
