...
const TAILNET_BIND_WARNING =
  "No Tailscale address was detected during setup. The saved config will stay on loopback until Tailscale is available or STAPLER_TAILNET_BIND_HOST is set.";

const ONBOARD_ENV_KEYS = [
  "STAPLER_PUBLIC_URL",
  "DATABASE_URL",
  "STAPLER_DB_BACKUP_ENABLED",
  "STAPLER_DB_BACKUP_INTERVAL_MINUTES",
  "STAPLER_DB_BACKUP_RETENTION_DAYS",
  "STAPLER_DB_BACKUP_DIR",
  "STAPLER_DEPLOYMENT_MODE",
  "STAPLER_DEPLOYMENT_EXPOSURE",
  "STAPLER_BIND",
  "STAPLER_BIND_HOST",
  "STAPLER_TAILNET_BIND_HOST",
  "HOST",
  "PORT",
  "SERVE_UI",
  "STAPLER_ALLOWED_HOSTNAMES",
  "STAPLER_AUTH_BASE_URL_MODE",
  "STAPLER_AUTH_PUBLIC_BASE_URL",
  "BETTER_AUTH_URL",
  "BETTER_AUTH_BASE_URL",
  "STAPLER_STORAGE_PROVIDER",
  "STAPLER_STORAGE_LOCAL_DIR",
  "STAPLER_STORAGE_S3_BUCKET",
  "STAPLER_STORAGE_S3_REGION",
  "STAPLER_STORAGE_S3_ENDPOINT",
  "STAPLER_STORAGE_S3_PREFIX",
  "STAPLER_STORAGE_S3_FORCE_PATH_STYLE",
  "STAPLER_SECRETS_PROVIDER",
  "STAPLER_SECRETS_STRICT_MODE",
  "STAPLER_SECRETS_MASTER_KEY_FILE",
] as const;
...
function quickstartDefaultsFromEnv(opts?: { preferTrustedLocal?: boolean }): {
  ...
  const publicUrl = preferTrustedLocal
    ? undefined
    : (
      process.env.STAPLER_PUBLIC_URL?.trim() ||
      process.env.STAPLER_AUTH_PUBLIC_BASE_URL?.trim() ||
  ...
  const deploymentMode = preferTrustedLocal
    ? "local_trusted"
    : (parseEnumFromEnv<DeploymentMode>(process.env.STAPLER_DEPLOYMENT_MODE, DEPLOYMENT_MODES) ?? "local_trusted");
  const deploymentExposureFromEnv = parseEnumFromEnv<DeploymentExposure>(
    process.env.STAPLER_DEPLOYMENT_EXPOSURE,
    DEPLOYMENT_EXPOSURES,
  );
  const deploymentExposure =
    deploymentMode === "local_trusted" ? "private" : (deploymentExposureFromEnv ?? "private");
  const bindFromEnv = parseEnumFromEnv<BindMode>(process.env.STAPLER_BIND, BIND_MODES);
  const customBindHostFromEnv = process.env.STAPLER_BIND_HOST?.trim() || undefined;
  ...
  const resolvedBind = resolveRuntimeBind({
    ...
    tailnetBindHost: process.env.STAPLER_TAILNET_BIND_HOST?.trim(),
  });
  const authPublicBaseUrl = publicUrl;
  const authBaseUrlModeFromEnv = parseEnumFromEnv<AuthBaseUrlMode>(
    process.env.STAPLER_AUTH_BASE_URL_MODE,
    AUTH_BASE_URL_MODES,
  );
  const authBaseUrlMode = authBaseUrlModeFromEnv ?? (authPublicBaseUrl ? "explicit" : "auto");
  const allowedHostnamesFromEnv = process.env.STAPLER_ALLOWED_HOSTNAMES
    ? process.env.STAPLER_ALLOWED_HOSTNAMES
      .split(",")
  ...
  const storageProvider =
    parseEnumFromEnv<StorageProvider>(process.env.STAPLER_STORAGE_PROVIDER, STORAGE_PROVIDERS) ??
    defaultStorage.provider;
  const secretsProvider =
    parseEnumFromEnv<SecretProvider>(process.env.STAPLER_SECRETS_PROVIDER, SECRET_PROVIDERS) ??
    defaultSecrets.provider;
  const databaseBackupEnabled = parseBooleanFromEnv(process.env.STAPLER_DB_BACKUP_ENABLED) ?? true;
  const databaseBackupIntervalMinutes = Math.max(
    1,
    parseNumberFromEnv(process.env.STAPLER_DB_BACKUP_INTERVAL_MINUTES) ?? 60,
  );
  const databaseBackupRetentionDays = Math.max(
    1,
    parseNumberFromEnv(process.env.STAPLER_DB_BACKUP_RETENTION_DAYS) ?? 30,
  );
  const defaults: OnboardDefaults = {
    database: {
      ...
      backup: {
        ...
        dir: resolvePathFromEnv(process.env.STAPLER_DB_BACKUP_DIR) ?? resolveDefaultBackupDir(instanceId),
      },
    },
    logging: {
      mode: "file",
    ...
    storage: {
      ...
      localDisk: {
        baseDir:
          resolvePathFromEnv(process.env.STAPLER_STORAGE_LOCAL_DIR) ?? defaultStorage.localDisk.baseDir,
      },
      s3: {
        bucket: process.env.STAPLER_STORAGE_S3_BUCKET ?? defaultStorage.s3.bucket,
        region: process.env.STAPLER_STORAGE_S3_REGION ?? defaultStorage.s3.region,
        endpoint: process.env.STAPLER_STORAGE_S3_ENDPOINT ?? defaultStorage.s3.endpoint,
        prefix: process.env.STAPLER_STORAGE_S3_PREFIX ?? defaultStorage.s3.prefix,
        forcePathStyle:
          parseBooleanFromEnv(process.env.STAPLER_STORAGE_S3_FORCE_PATH_STYLE) ??
          defaultStorage.s3.forcePathStyle,
      },
    },
    secrets: {
      provider: secretsProvider,
      strictMode: parseBooleanFromEnv(process.env.STAPLER_SECRETS_STRICT_MODE) ?? defaultSecrets.strictMode,
      localEncrypted: {
        keyFilePath:
          resolvePathFromEnv(process.env.STAPLER_SECRETS_MASTER_KEY_FILE) ??
  ...
  if (deploymentMode === "local_trusted" && process.env.STAPLER_DEPLOYMENT_EXPOSURE !== undefined) {
    ignoredEnvKeys.push({
      key: "STAPLER_DEPLOYMENT_EXPOSURE",
...
export async function onboard(opts: OnboardOptions): Promise<void> {
  ...
  if (existingConfig) {
    ...
    if (jwtSecret.created) {
      p.log.success(`Created ${pc.cyan("STAPLER_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
    } else if (process.env.STAPLER_AGENT_JWT_SECRET?.trim()) {
      p.log.info(`Using existing ${pc.cyan("STAPLER_AGENT_JWT_SECRET")} from environment`);
    } else {
      p.log.info(`Using existing ${pc.cyan("STAPLER_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
    }
    ...
    p.note(
      [
        ...
        "Agent auth: STAPLER_AGENT_JWT_SECRET configured",
      ].join("\n"),
      "Configuration ready",
    );
    ...
    if (shouldRunNow && !opts.invokedByRun) {
      process.env.STAPLER_OPEN_ON_LISTEN = "true";
  ...
  if (setupMode === "advanced") {
    ...
    if (database.mode === "postgres" && database.connectionString) {
      ...
      try {
        const { createDb } = await import("@stapler/db");
