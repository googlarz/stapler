...
import { ApiRequestError, StaplerApiClient } from "../../client/http.js";
...
export function resolveCommandContext(
  ...
  const explicitApiKey =
    options.apiKey?.trim() ||
    process.env.STAPLER_API_KEY?.trim() ||
  ...
  const companyId =
    ...
    process.env.STAPLER_COMPANY_ID?.trim() ||
    profile.companyId;

  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set STAPLER_COMPANY_ID, or set context profile companyId via `paperclipai context set`.",
    );
  }

  const api = new StaplerApiClient({
    apiBase,
    apiKey,
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.STAPLER_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.STAPLER_SERVER_PORT || "");
