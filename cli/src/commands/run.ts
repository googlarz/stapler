...
function resolveBootstrapInviteBaseUrl(
  ...
  const explicitBaseUrl =
    process.env.STAPLER_PUBLIC_URL ??
...
function maybeEnableUiDevMiddleware(entrypoint: string): void {
  if (process.env.STAPLER_UI_DEV_MIDDLEWARE !== undefined) return;
  const normalized = entrypoint.replaceAll("\\", "/");
  if (normalized.endsWith("/server/src/index.ts") || normalized.endsWith("@stapler/server/src/index.ts")) {
    process.env.STAPLER_UI_DEV_MIDDLEWARE = "true";
  }
}
...
async function importServerEntry(): Promise<StartedServer> {
  ...
  // Production mode: import the published @stapler/server package
  try {
    const mod = await import("@stapler/server");
    return await startServerFromModule(mod, "@stapler/server");
  } catch (err) {
    const missingSpecifier = getMissingModuleSpecifier(err);
    const missingServerEntrypoint = !missingSpecifier || missingSpecifier === "@stapler/server";
    if (isModuleNotFoundError(err) && missingServerEntrypoint) {
      throw new Error(
        `Could not locate a Paperclip server entrypoint.\n` +
          `Tried: ${devEntry}, @stapler/server\n` +
          `${formatError(err)}`,
      );
    }
    throw new Error(
