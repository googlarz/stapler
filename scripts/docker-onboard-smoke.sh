...
STAPLER_DEPLOYMENT_MODE="${STAPLER_DEPLOYMENT_MODE:-authenticated}"
STAPLER_DEPLOYMENT_EXPOSURE="${STAPLER_DEPLOYMENT_EXPOSURE:-private}"
STAPLER_PUBLIC_URL="${STAPLER_PUBLIC_URL:-http://localhost:${HOST_PORT}}"
...
generate_bootstrap_invite_url() {
  ...
  if bootstrap_output="$(
    docker exec \
      -e STAPLER_DEPLOYMENT_MODE="$STAPLER_DEPLOYMENT_MODE" \
      -e STAPLER_DEPLOYMENT_EXPOSURE="$STAPLER_DEPLOYMENT_EXPOSURE" \
      -e STAPLER_PUBLIC_URL="$STAPLER_PUBLIC_URL" \
      -e STAPLER_HOME="/paperclip" \
      "$CONTAINER_NAME" bash -lc \
      'timeout 20s npx --yes "paperclipai@${PAPERCLIPAI_VERSION}" auth bootstrap-ceo --data-dir "$STAPLER_HOME" --base-url "$STAPLER_PUBLIC_URL"' \
      2>&1
  )"; then
    bootstrap_status=0
  else
...
sign_up_or_sign_in() {
  ...
  signup_status="$(post_json_with_cookies \
    "$STAPLER_PUBLIC_URL/api/auth/sign-up/email" \
  ...
  signin_status="$(post_json_with_cookies \
    "$STAPLER_PUBLIC_URL/api/auth/sign-in/email" \
...
auto_bootstrap_authenticated_smoke() {
  local health_url="$STAPLER_PUBLIC_URL/api/health"
  local health_json
  ...
  if [[ "$health_json" == *'"bootstrapStatus":"ready"'* ]]; then
    ...
    accept_status="$(post_json_with_cookies \
      "$STAPLER_PUBLIC_URL/api/invites/$invite_token/accept" \
      '{"requestType":"human"}' \
      "$accept_response")"
  ...
  local session_json
  session_json="$(get_with_cookies "$STAPLER_PUBLIC_URL/api/auth/get-session")"
  ...
  companies_json="$(get_with_cookies "$STAPLER_PUBLIC_URL/api/companies")"
...
echo "    Public URL: $STAPLER_PUBLIC_URL"
echo "    Smoke auto-bootstrap: $SMOKE_AUTO_BOOTSTRAP"
echo "    Detached mode: $SMOKE_DETACH"
echo "    Data dir: $DATA_DIR"
echo "    Deployment: $STAPLER_DEPLOYMENT_MODE/$STAPLER_DEPLOYMENT_EXPOSURE"
...
if ! wait_for_http "$STAPLER_PUBLIC_URL/api/health" 90 1; then
  echo "Smoke bootstrap failed: server did not become ready at $STAPLER_PUBLIC_URL/api/health" >&2
  exit 1
fi

if [[ "$SMOKE_AUTO_BOOTSTRAP" == "true" && "$STAPLER_DEPLOYMENT_MODE" == "authenticated" ]]; then
  auto_bootstrap_authenticated_smoke
fi
...
if [[ "$SMOKE_DETACH" == "true" ]]; then
  ...
  echo "    Smoke base URL: $STAPLER_PUBLIC_URL"
  echo "    Smoke admin credentials: $SMOKE_ADMIN_EMAIL / $SMOKE_ADMIN_PASSWORD"
