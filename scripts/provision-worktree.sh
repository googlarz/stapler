...
base_cwd="${STAPLER_WORKSPACE_BASE_CWD:?STAPLER_WORKSPACE_BASE_CWD is required}"
worktree_cwd="${STAPLER_WORKSPACE_CWD:?STAPLER_WORKSPACE_CWD is required}"
paperclip_home="${STAPLER_HOME:-$HOME/.paperclip}"
paperclip_instance_id="${STAPLER_INSTANCE_ID:-default}"
paperclip_dir="$worktree_cwd/.paperclip"
worktree_config_path="$paperclip_dir/config.json"
worktree_env_path="$paperclip_dir/.env"
worktree_name="${STAPLER_WORKSPACE_BRANCH:-$(basename "$worktree_cwd")}"
...
source_config_path="${STAPLER_CONFIG:-}"
...
write_fallback_worktree_config() {
  ...
  STAPLER_DIR="$paperclip_dir" \
  SOURCE_CONFIG_PATH="$source_config_path" \
  SOURCE_ENV_PATH="$source_env_path" \
  STAPLER_WORKTREES_DIR="${STAPLER_WORKTREES_DIR:-}" \
  node <<'EOF'
...
async function main() {
  ...
  const paperclipDir = process.env.STAPLER_DIR;
  const sourceConfigPath = process.env.SOURCE_CONFIG_PATH;
  const sourceEnvPath = process.env.SOURCE_ENV_PATH;
  const worktreeHome = path.resolve(expandHomePrefix(nonEmpty(process.env.STAPLER_WORKTREES_DIR) ?? "~/.paperclip-worktrees"));
  ...
  const inlineMasterKey = nonEmpty(sourceEnvEntries.STAPLER_SECRETS_MASTER_KEY);
  if (inlineMasterKey) {
    ...
    const sourceKeyFilePath = nonEmpty(sourceEnvEntries.STAPLER_SECRETS_MASTER_KEY_FILE)
      ? resolveRuntimeLikePath(sourceEnvEntries.STAPLER_SECRETS_MASTER_KEY_FILE, sourceConfigPath)
  ...
  const envLines = [
    "STAPLER_HOME=" + JSON.stringify(worktreeHome),
    "STAPLER_INSTANCE_ID=" + JSON.stringify(instanceId),
    "STAPLER_CONFIG=" + JSON.stringify(configPath),
    "STAPLER_CONTEXT=" + JSON.stringify(path.resolve(worktreeHome, "context.json")),
    "STAPLER_IN_WORKTREE=true",
    "STAPLER_WORKTREE_NAME=" + JSON.stringify(worktreeName),
  ];

  const agentJwtSecret = nonEmpty(sourceEnvEntries.STAPLER_AGENT_JWT_SECRET);
  if (agentJwtSecret) {
    envLines.push("STAPLER_AGENT_JWT_SECRET=" + JSON.stringify(agentJwtSecret));
  }
