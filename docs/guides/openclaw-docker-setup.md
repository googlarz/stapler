...
- `STAPLER_HOST_PORT` (default `3100`)
- `STAPLER_HOST_FROM_CONTAINER` (default `host.docker.internal`)
...
```bash
STAPLER_AUTH_HEADER="Bearer <token>" pnpm smoke:openclaw-join
# or
STAPLER_COOKIE="your_session_cookie=..." pnpm smoke:openclaw-join
```
