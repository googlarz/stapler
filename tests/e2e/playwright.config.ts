...
const STAPLER_HOME = fs.mkdtempSync(path.join(os.tmpdir(), "paperclip-e2e-home-"));
...
export default defineConfig({
  ...
  // `onboard --yes --run` works in a non-interactive temp STAPLER_HOME.
  webServer: {
    ...
    env: {
      ...
      PORT: String(PORT),
      STAPLER_HOME,
      STAPLER_INSTANCE_ID: "playwright-e2e",
      STAPLER_BIND: "loopback",
